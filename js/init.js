	...
	var FrenifyNeoh = {
		...
		productModal: function(){
			...
			$('.neoh_fn_drops .item a').off().on('click', function(){
				...
				var TelegramURL	= parent.data('modal-Telegram-url');
				
				// attach empty if not setted
				if(!openseaURL){openseaURL = '';}
				if(!TelegramURL){TelegramURL = '';}
				...
				// change Telegram URL or disable opensea button
				if(TelegramURL === ''){
					modal.find('.Telegram').addClass('disable');
				}else{
					modal.find('.Telegram').attr('href',TelegramURL);
				}
